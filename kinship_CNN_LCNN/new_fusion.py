"""
═══════════════════════════════════════════════════════════════════════════════
  DeepZigZag Fusion LCNN  —  Best Configuration
  Dataset  : KinFaceW-II
  Features : ArcFace + FaceNet + ResNet50 + VGGFace + ResNet101 (deep)
             + HistZigZag (handcrafted, 9-channel LBP, zigzag scan)
  Split    : 5-Fold Cross-Validation (80% train / 10% val / 10% test)
  Results  : ~87% overall accuracy

  Architecture:
    ┌─ 5 LocalBranch (one per deep model, from deep_lcnn 83%) ─┐
    │   ArcFace | FaceNet | ResNet50 | VGGFace | ResNet101      │
    └────────────────────────┬──────────────────────────────────┘
                             │ concat → fusion MLP (512→256→128)
    ┌─ ZigZag Branch (from kinship-Lcnn 87%) ──────────────────┐
    │   HistZigZag (diff+prod+cos+chi²) → 512→256 + SEBlock    │
    └────────────────────────┬──────────────────────────────────┘
                             │ concat (deep_fused + zz_out)
                             └──→ Final head → sigmoid → kinship
═══════════════════════════════════════════════════════════════════════════════
"""
...
OUTPUT_DIR    = r"C:\Users\surface laptop 5\OneDrive\Documents\PFE\Methodes_classiques\Hist-LDZP\HistZigZag\ps16_ss8"
...
REL_HP = {
    "FD": dict(
        epochs=250, lr=6e-4, weight_decay=5e-4, patience=50,
...
def build_zigzag_pair(feats: np.ndarray, idxa, idxb) -> np.ndarray:
    """
    From kinship-Lcnn (87%): sqrt mean-centered diff + signed-sqrt prod
    + cosine + chi² (power-normalized).
    """
    a = feats[idxa].astype(np.float64)
    b = feats[idxb].astype(np.float64)
...
# ══════════════════════════════════════════════════════════════════════════════
#  NORMALISATION  (RobustScaler — best from deep_lcnn for multi-modal)
# ══════════════════════════════════════════════════════════════════════════════
...
def main():
    ...
    print("  Scaler: RobustScaler  |  TTA runs=7  |  Mixup + CutMix (FD)")
    print("="*70)
