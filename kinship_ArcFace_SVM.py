...
def find_best_C(X_train, y_train, train_fold_ids, c_grid):
    ...
    for C in c_grid:
        ...
        for inner_f in inner_folds:
            ...
            clf = SVC(kernel='linear ', C=C, gamma='scale', random_state=42)
            clf.fit(Xtr, ytr)
...
for relation, paths in RELATIONS.items():
    ...
    for f in range(1, 6):
        ...
        # ArcFace embeddings are already L2-normalized so we use StandardScaler
        # (not MinMax) to keep the embedding geometry intact
...
print(f"\n{'='*55}")
print("  ACCURACY GLOBALE — ArcFace + SVM (RBF)")
print(f"{'='*55}")
...
print(f"  ─────────────────────────────────────────────────────")
print(f"  ArcFace + SVM Accuracy : {overall_acc * 100:.2f}%")