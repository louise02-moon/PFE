...
# VGG-Face: 2622 dims — search smaller k since it responded well to low k
VGG_K_GRID  = [50, 100, 200, 300, 500, 700, 1000]

# HLBP: 58667 dims — much larger space, search a wider range
# Even keeping 5000 out of 58667 is aggressive selection (top 8.5%)
HLBP_K_GRID = [500, 1000, 2000, 3000, 5000]


# ─── Fisher Score ─────────────────────────────────────────────────────────────
# For each feature dimension, computes how well it separates kin from non-kin.
# Always fitted on training data only — never on test data.
...
def run_experiment():
    ...
    for relation, paths in RELATIONS.items():
        ...
        for f in range(1, 6):
            ...
            # ── Step 1: tune VGG-Face (Fisher Score + C) via inner CV ─────────
            C_vgg, k_vgg = find_best_C_and_k(
                X_vgg_tr, y_train, fold_ids, C_GRID, VGG_K_GRID, center
            )

            # ── Step 2: tune HLBP (Fisher Score + C) via inner CV ────────────
            # NEW: Fisher Score now applied to HLBP too.
            # With 58,667 dims, even keeping top 5,000 removes 91% of the noise.
            C_hlbp, k_hlbp = find_best_C_and_k(
                X_hlbp_tr, y_train, fold_ids, C_GRID, HLBP_K_GRID, power_normalize
            )

            # ── Step 3: get decision scores from both SVMs ────────────────────
            vgg_tr_scores,  vgg_te_scores  = get_scores(
                X_vgg_tr,  X_vgg_te,  y_train, center,          C_vgg,  k_vgg)
            hlbp_tr_scores, hlbp_te_scores = get_scores(
                X_hlbp_tr, X_hlbp_te, y_train, power_normalize, C_hlbp, k_hlbp)

            # ── Step 4: build and scale meta-features ─────────────────────────
            meta_train = np.column_stack([vgg_tr_scores, hlbp_tr_scores])
            meta_test  = np.column_stack([vgg_te_scores, hlbp_te_scores])
            ...
            print(f"  Fold {f}: {acc*100:.2f}%  "
                  f"(C_vgg={C_vgg}, k_vgg={k_vgg}, "
                  f"C_hlbp={C_hlbp}, k_hlbp={k_hlbp}, "
                  f"meta_C={best_meta_C}, "
    ...
    # ─── Summary ──────────────────────────────────────────────────────────────
    print(f"\n{'='*60}")
    print("  FINAL SUMMARY — VGGFace + HLBP, Fisher Score on Both")
    print(f"{'='*60}")
