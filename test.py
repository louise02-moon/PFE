...
#  Models: VGGFace + VGG19 + ResNet50 + ResNet101
...
ZZ_DIR       = r"C:\Users\surface laptop 5\OneDrive\Documents\PFE\Hist-LDZP\HistZigZag\hsv_ycbcr"
...
DEEP_PATHS = {
    rel: {
        "vgg19"   : os.path.join(VGG19_DIR,    f"VGG19_{rel}.pkl"),
        "resnet50" : os.path.join(RESNET50_DIR, f"ResNet50_{rel}.pkl"),
        "resnet101": os.path.join(RESNET101_DIR, f"ResNet101_{rel}.pkl"),
...
def build_all_pairs(vgg19, res50, res101, vggface, zz, idxa, idxb):
    vgg19_p = build_deep_pairs_single(vgg19,   idxa, idxb)
    res50_p = build_deep_pairs_single(res50,   idxa, idxb)
    res101_p= build_deep_pairs_single(res101,  idxa, idxb)
    vgg_p   = build_deep_pairs_single(vggface, idxa, idxb)
    zz_p  = build_zz_pairs(zz,               idxa, idxb)

    X_deep = zscore_normalize(np.concatenate([vgg19_p, res50_p, res101_p, vgg_p], axis=1))
...
def main():
    ...
    for rel in RELATIONS:
        ...
        dp        = DEEP_PATHS[rel]
        vgg19_feats = load_deep_embedding(dp["vgg19"])
        res50_feats = load_deep_embedding(dp["resnet50"])
        res101_feats= load_deep_embedding(dp["resnet101"])
        vgg_feats   = load_deep_embedding(dp["vggface"])

        print(f"  HistZigZag : {zz_feats.shape}")
        print(f"  VGG19      : {vgg19_feats.shape}")
        print(f"  ResNet50   : {res50_feats.shape}")
        print(f"  ResNet101  : {res101_feats.shape}")
        ...
        X, deep_dim, zz_dim = build_all_pairs(
            vgg19_feats, res50_feats, res101_feats, vgg_feats, zz_feats, idxa, idxb
        )
