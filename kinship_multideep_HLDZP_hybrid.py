"""
kinship_multideep_HLDZP_hybrid_v2.py
────────────────────────────────────────────────────────────────────────────────
ArcFace + FaceNet + ResNet50 + VGGFace  +  HistLDZP similarity scores  →  LCNN

WHY THE PREVIOUS VERSION GOT ~76%
──────────────────────────────────
The linear SVM was being trained on 79,297–122,305 dimensional LDZP pair
features with only ~400 training samples — a 200–300× dimensions-to-samples
ratio. A well-conditioned linear SVM needs this below ~10×. At 300× the SVM
overfits completely on training noise and generalizes poorly.

Adding more channels (9 instead of 4) makes this 687× worse — wrong direction.
