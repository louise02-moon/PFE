...
PKL_DIR = r"C:\Users\surface laptop 5\OneDrive\Documents\PFE\Apprentissage profond\VGGFace\vggface_embeddings"
MAT_DIR = r"C:\Users\surface laptop 5\OneDrive\Documents\PFE\lbp"

RELATIONS = {
    "Father-Daughter": {
        "pkl": f"{PKL_DIR}\\VGG16_FD.pkl",
        "mat": f"{MAT_DIR}\\LBP_fd.mat",
    },
    "Father-Son": {
        "pkl": f"{PKL_DIR}\\VGG16_FS.pkl",
        "mat": f"{MAT_DIR}\\LBP_fs.mat",
    },
    "Mother-Daughter": {
        "pkl": f"{PKL_DIR}\\VGG16_MD.pkl",
        "mat": f"{MAT_DIR}\\LBP_md.mat",
    },
    "Mother-Son": {
        "pkl": f"{PKL_DIR}\\VGG16_MS.pkl",
...
# ─── Accuracy globale VGG16/VGGFace (moyenne des 4 relations) ────────────────
overall_acc = np.mean([res['mean_accuracy'] for res in all_results.values()])

print(f"\n{'='*50}")
print(f"  ACCURACY GLOBALE — VGG16 (VGGFACE)")
print(f"{'='*50}")
...
print(f"  ─────────────────────────────────────")
print(f"  VGG16 (VGGFace) Accuracy : {overall_acc * 100:.2f}%")