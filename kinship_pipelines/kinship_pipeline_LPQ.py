...
PKL_DIR = r"C:\Users\surface laptop 5\OneDrive\Documents\PFE\Methodes classiques\LBP Gray\LBP_gray_feature_vectors"
MAT_DIR = r"C:\Users\surface laptop 5\OneDrive\Documents\PFE\lbp"  

RELATIONS = {
    "Father-Daughter": {
        "pkl": f"{PKL_DIR}\\LBP_FD.pkl",
        "mat": f"{MAT_DIR}\\LBP_fd.mat",
    },
    "Father-Son": {
        "pkl": f"{PKL_DIR}\\LBP_FS.pkl",
        "mat": f"{MAT_DIR}\\LBP_fs.mat",
    },
    "Mother-Daughter": {
        "pkl": f"{PKL_DIR}\\LBP_MD.pkl",
        "mat": f"{MAT_DIR}\\LBP_md.mat",
    },
    "Mother-Son": {
        "pkl": f"{PKL_DIR}\\LBP_MS.pkl",
...
for relation, paths in RELATIONS.items():
    ...
    for f in range(1, 6):
        ...
        # Reset seed à chaque fold pour reproductibilité
        random.seed(42)
        np.random.seed(42)
        tf.random.set_seed(42)
...
# ─── Accuracy globale LBP Gray (moyenne des 4 relations) ────────────────────
overall_acc = np.mean([res['mean_accuracy'] for res in all_results.values()])

print(f"\n{'='*50}")
print(f"  ACCURACY GLOBALE — LBP GRAY")
print(f"{'='*50}")
...
print(f"  ─────────────────────────────────────")
print(f"  LBP Gray Accuracy : {overall_acc * 100:.2f}%")