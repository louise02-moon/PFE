...
# ─── Power normalization — exact same as Hist-LBP pipeline ───────────────────
def power_normalization(X_train, X_test):
    ...
    # Step 3: mean centering — fit on train only
...
# ─── Best C by inner CV — exact same as Hist-LBP pipeline ────────────────────
C_GRID = [0.01, 0.1, 1, 10, 100, 1000]

def find_best_C(X_train, y_train, train_fold_ids, c_grid):
    ...
    for C in c_grid:
        ...
        for inner_f in inner_folds:
            ...
            Xtr, Xte = X_train[imask_tr], X_train[imask_te]
            ytr, yte = y_train[imask_tr], y_train[imask_te]

            # Apply power norm inside inner CV too
...
for relation, paths in RELATIONS.items():
    ...
    print(f"  Embedding shape : {ux.shape}")
    ...
    # Absolute difference — same fusion as Hist-LBP
    ...
    for f in range(1, 6):
        ...
        # Power normalization — fit on train only
        X_train, X_test = power_normalization(X_train_raw, X_test_raw)

        # Linear SVM — exact same as Hist-LBP pipeline
...
print(f"\n{'='*55}")
print("  ACCURACY GLOBALE — FaceNet + Linear SVM + Power Norm")
print(f"{'='*55}")
...
print(f"  ─────────────────────────────────────────────────────")
print(f"  FaceNet + Linear SVM + Power Norm : {overall_acc * 100:.2f}%")